	...
	export default function XiuXianLunDaoApp() {
	  // mounted gate（避免 SSR 與 CSR 初次渲染不一致）
	  ...
	  const talentAutoBonus = s.talent.auto * 0.1;
	  const talentClickBonus = s.talent.click * 0.1;
	  ...
	  const refineStones = () => {
		if (s.qi < QI_TO_STONE) {
		  setMsg("靈力不足，至少需要100靈力才能煉化為 1 枚靈石。");
		  return;
		}
	  ...
	  const buyArtifact = (ak) => {
		...
		if (s.realmIndex < a.unlockRealmIndex) {
		  setMsg("境界未到，無法驅使此法寶。");
		  return;
		}
		if (s.stones < a.cost) {
	  ...
	  const tryBreakthrough = (useDaoHeart = false) => {
		...
		const bonus = (useDaoHeart ? 0.1 : 0) + artBreakBonus;
	  ...
	  const clickTrain = () => {
		setClickedFx(true);
		cultivate();
		setTimeout(() => setClickedFx(false), 120);
	  };
	  ...
	  return (
		<div className="min-h-screen bg-gradient-to-b from-slate-900 via-gray-900 to-black text-slate-100 p-4 md:p-8">
			  ...
			  <div className="flex flex-wrap gap-2 mt-3">
				<button
				  ...
				  className={`px-4 py-2 rounded-lg bg-indigo-700 hover:bg-indigo-600 active:scale-95 transition ${
					clickedFx ? "ring-4 ring-amber-400/60" : ""
				  }`}
				>
				  修煉
				</button>
				<button onClick={refineStones} className="px-4 py-2 rounded-lg bg-emerald-700 hover:bg-emerald-600">
				  煉化靈石
				</button>
				<button onClick={comprehendDao} className="px-4 py-2 rounded-lg bg-amber-700 hover:bg-amber-600">
				  參悟道心
				</button>
			  </div>
			</Card>
			  ...
			  <ul className="space-y-2 text-sm">
				{["tuna", "wuxing", "jiutian"].map((k) => {
				  ...
				  return (
					<li
					  key={k}
					  className="flex items-center justify-between gap-3 p-2 rounded-lg bg-black/30 border border-slate-700/40"
					>
					  <div>
						<div className="font-medium">
						  {def.name} <span className="opacity-70">Lv.{lv}</span>
						</div>
					  ...
					  <button
						...
						className={`px-3 py-1.5 rounded-lg ${
						  can ? "bg-sky-700 hover:bg-sky-600" : "bg-slate-700 cursor-not-allowed"
						}`}
					  >
						升級（{fmt(cost)} 石）
					  </button>
				  ...
				  return (
					...
					>
					  <div>
						<div className="font-medium">
						  {a.name}
						  {owned && <span className="ml-2 text-emerald-400">（已持有）</span>}
						</div>
						<div className="text-slate-400 text-xs">{a.desc}</div>
						{!unlocked && (
						  <div className="text-xs text-amber-300 mt-1">需達 {REALMS[a.unlockRealmIndex]?.name} 解鎖</div>
						)}
					  </div>
					  <button
						...
						disabled={!canBuy}
						className={`px-3 py-1.5 rounded-lg ${
						  canBuy ? "bg-purple-700 hover:bg-purple-600" : "bg-slate-700 cursor-not-allowed"
						}`}
					  >
			...
			<Card title="突破境界">
			  {nextRealm ? (
				<>
				  ...
				  <div className="flex flex-wrap gap-2 mt-3">
					<button onClick={() => tryBreakthrough(false)} className="px-4 py-2 rounded-lg bg-rose-700 hover:bg-rose-600">
					  嘗試突破
					</button>
					<button onClick={() => tryBreakthrough(true)} className="px-4 py-2 rounded-lg bg-amber-700 hover:bg-amber-600">
					  道心輔助突破（-1道心）
					</button>
				  </div>
				</>
			  ...
			  <div className="mt-3">
				<button
				  ...
				  className={`px-4 py-2 rounded-lg ${
					canAscend ? "bg-emerald-700 hover:bg-emerald-600" : "bg-slate-700 cursor-not-allowed"
				  }`}
				>
		  ...
		  <section className="max-w-6xl mx-auto mt-6 grid md:grid-cols-2 gap-6">
			<Card title="存檔 / 匯入">
			  <div className="flex flex-wrap gap-2">
				<button onClick={exportSave} className="px-4 py-2 rounded-lg bg-sky-700 hover:bg-sky-600">
				  匯出到剪貼簿
				</button>
				<button onClick={hardReset} className="px-4 py-2 rounded-lg bg-rose-800 hover:bg-rose-700">
				  重置存檔
				</button>
			  </div>
			  <div className="mt-3">
				...
				/>
				<button onClick={importSave} className="mt-2 px-4 py-2 rounded-lg bg-amber-700 hover:bg-amber-600">
				  匯入存檔
				</button>
			  </div>
			</Card>
		  ...
		  <footer className="max-w-6xl mx-auto text-center mt-10 text-xs text-slate-500" suppressHydrationWarning>
	...
	function Leaderboard({ s }) {
	  ...
	  const board = (() => {
		try {
		  return JSON.parse(localStorage.getItem("xiuxian-leaderboard") || "[]");
		} catch (e) {
		  return [];
		}
	  })();
		...
		<Card title="排行榜（本地）">
		  <div className="flex items-center gap-2 mb-3">
			<input
			  ...
			  placeholder="取個道號…"
			  className="flex-1 px-3 py-1.5 rounded-lg bg-black/40 border border-slate-700 outline-none text-sm"
			/>
			<button onClick={saveScore} className="px-3 py-1.5 rounded-lg bg-indigo-700 hover:bg-indigo-600 text-sm">
			  提交成績
			</button>
		  </div>
	  ...
	  return (
		<div className="relative max-w-6xl mx-auto mt-6 overflow-hidden rounded-2xl border border-white/10 bg-[#0f172a]">
		  ...
		  <style>{`
			...
			.vortex{ position:absolute; left:50%; top:50%; transform:translate(-50%,-50%); border-radius:9999px;
					 background:conic-gradient(from 0deg, rgba(255,255,255,.0) 0deg, rgba(255,255,255,.55) 30deg, rgba(255,255,255,.0) 120deg, rgba(255,255,255,.0) 360deg);
					 filter: blur(6px); animation: spin 18s linear infinite;
			...
			.spark{ position:absolute; left:50%; top:50%; width:3px; height:3px; background:#fff; border-radius:9999px;
					transform-origin: -var(--r) 0; animation: orbit 6s linear infinite, twinkle 3.2s ease-in-out infinite; }
		  `}</style>
		</div>
	  );
	}
		...
		<div className="fixed inset-0 z-50 flex items-center justify-center">
		  ...
		  <div className="relative w-full max-w-lg mx-auto rounded-2xl border border-indigo-500/40 bg-slate-900 shadow-2xl p-5">
			...
			<div className="grid grid-cols-3 gap-2 my-3">
			  {Array.from({ length: 9 }).map((_, i) => {
				...
				return (
				  <div
					key={i}
					className={`h-16 rounded-xl border flex items-center justify-center text-lg font-semibold
					${row ? (row.pass ? "bg-emerald-600/30 border-emerald-400/40 text-emerald-200" : "bg-rose-600/30 border-rose-400/40 text-rose-200") : "bg-slate-800/60 border-slate-600/50 text-slate-300"}
			...
			<div className="flex items-center justify-end gap-2">
			  {!finished && (
				<button onClick={start} disabled={running} className={`px-4 py-2 rounded-lg ${running ? "bg-slate-700 cursor-not-allowed" : "bg-indigo-700 hover:bg-indigo-600"}`}>
				  {running ? "渡劫中…" : "開始渡劫"}
				</button>
			  )}
			  {finished && (
				<button onClick={() => setState((p) => ({ ...p, open: false }))} className="px-4 py-2 rounded-lg bg-emerald-700 hover:bg-emerald-600">
				  確定
				</button>
			  )}
			</div>
	  ...
	  return (
		<Card title="開發者工具（內部測試）">
		  <div className="grid grid-cols-3 gap-2 text-sm">
			<button
			  onClick={() => {
				setS((p) => ({ ...p, stones: p.stones + 10000 }));
				setMsg("測試加值：靈石 +10,000");
			  }}
			  className="py-2 rounded-xl bg-slate-800 hover:bg-slate-700"
			>
			  +10,000 靈石
			</button>
			<button
			  onClick={() => {
				setS((p) => ({ ...p, qi: p.qi + 100000 }));
				setMsg("測試加值：靈力 +100,000");
			  }}
			  className="py-2 rounded-xl bg-slate-800 hover:bg-slate-700"
			>
			  +100,000 靈力
			...
			<button
			  onClick={() => {
				setS((p) => ({ ...p, daoHeart: p.daoHeart + 5 }));
				setMsg("測試加值：道心 +5");
			  }}
			  className="py-2 rounded-xl bg-slate-800 hover:bg-slate-700"
			>
