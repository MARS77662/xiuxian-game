	...
	import EventCenter from "./components/EventCenter.jsx";
	import { isQiOverheat } from "./lib/qiOverflow";
	...
	function Story({ profile, onFinish }){
	  ...
	  theScene: {
		// no-op label, just for structure clarity
	  }
	...
	function Hub({ profile, onEnterCultivate }){
	  ...
	  const [tab, setTab] = useState("sect"); // sect | quests | explore | demon | market
		...
		return () => clearInterval(id);
			 ...
			 function firstNum(obj, keys=[], def=0){
		  for (const k of keys){
			const v = obj?.[k];
		  ...
		  return def;
			...
			// app/xiuxian/page.jsx 內，取代原本 buildUserInfoState
		function buildUserInfoState(profile){
		  let save = null;
		  try { save = JSON.parse(localStorage.getItem(SAVE_KEY) || "null"); } catch {}
		  ...
		  return {
			nickname: profile?.name || "無名散修",
			sect: profile?.sectName || profile?.faction || "散修",
			realm,                 // 正確顯示境界名稱（例如：元嬰）
			realmIndex: idx,       // 傳給 UserInfo 作為備援推導
			qi:  Number(save?.qi ?? 0),
			qiMax,
			xp:  Number(save?.xp ?? 0),
			xpMax,
	...
	export default function XiuxianPage(){
	  ...
	  useEffect(() => {
		try {
		  ...
		  if (profRaw) setPlayer(JSON.parse(profRaw));
